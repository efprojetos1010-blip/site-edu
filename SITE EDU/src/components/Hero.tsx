...
const Hero = () => {
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden bg-background">
      ...
      {/* Large vertical accent band - right side */}
      ...
      {/* Large geometric diamond cluster - behind candidate */}
      <svg className="absolute top-[8%] right-[15%] w-80 h-80 opacity-[0.15]" viewBox="0 0 300 300" fill="none" aria-hidden="true">
        <rect x="110" y="110" width="80" height="80" transform="rotate(45 150 150)" stroke="hsl(var(--primary))" strokeWidth="3" fill="none" />
        <rect x="125" y="125" width="50" height="50" transform="rotate(45 150 150)" stroke="hsl(var(--accent))" strokeWidth="2.5" fill="none" />
        <rect x="138" y="138" width="24" height="24" transform="rotate(45 150 150)" fill="hsl(var(--accent))" opacity="0.4" />
        <circle cx="150" cy="150" r="130" stroke="hsl(var(--primary))" strokeWidth="1.5" strokeDasharray="8 6" opacity="0.4" />
        <circle cx="150" cy="150" r="110" stroke="hsl(var(--accent))" strokeWidth="1" strokeDasharray="4 8" opacity="0.3" />
      </svg>

      {/* Vertical geometric strip - far right */}
      <svg className="absolute right-0 top-[10%] w-16 h-[80%] opacity-[0.2]" viewBox="0 0 50 500" fill="none" aria-hidden="true">
        {Array.from({ length: 14 }).map((_, i) => (
          <g key={i} transform={`translate(0, ${i * 35})`}>
            <rect
              x="10" y="4" width="18" height="18"
              transform={`rotate(45 19 13)`}
              fill={i % 4 === 0 ? "hsl(var(--primary))" : i % 4 === 1 ? "hsl(var(--accent))" : i % 4 === 2 ? "hsl(var(--orange))" : "hsl(var(--destructive))"}
              opacity="0.7"
            />
            <rect
              x="28" y="16" width="10" height="10"
      ...
      {/* Second vertical strip - inner */}
      <svg className="absolute right-[6%] top-[5%] w-10 h-[70%] opacity-[0.12]" viewBox="0 0 30 400" fill="none" aria-hidden="true">
        {Array.from({ length: 10 }).map((_, i) => (
          <g key={i} transform={`translate(0, ${i * 40})`}>
            <circle cx="15" cy="15" r="8" stroke="hsl(var(--primary))" strokeWidth="1.5" fill="none" opacity="0.6" />
            <circle cx="15" cy="15" r="3" fill="hsl(var(--accent))" opacity="0.5" />
          </g>
        ))}
      </svg>

      {/* Scattered colorful blocks */}
      ...
      {/* Decorative circles */}
      <svg className="absolute top-[30%] left-[3%] w-32 h-32 opacity-[0.07]" viewBox="0 0 100 100" fill="none" aria-hidden="true">
        <circle cx="50" cy="50" r="45" stroke="hsl(var(--accent))" strokeWidth="2" />
        <circle cx="50" cy="50" r="30" stroke="hsl(var(--orange))" strokeWidth="1.5" strokeDasharray="5 4" />
        <circle cx="50" cy="50" r="12" fill="hsl(var(--primary))" opacity="0.2" />
      </svg>

      {/* Spiral motif */}
      <svg className="absolute bottom-[30%] right-[28%] w-16 h-16 opacity-[0.08]" viewBox="0 0 60 60" fill="none" aria-hidden="true">
        <path d="M30 30 C30 25 35 20 40 20 C45 20 50 25 50 30 C50 40 40 45 30 45 C15 45 10 35 10 25 C10 10 25 5 35 5" stroke="hsl(var(--primary))" strokeWidth="2.5" strokeLinecap="round" />
      </svg>

      {/* Araucária silhouette - subtle */}
      <svg className="absolute bottom-[10%] left-[4%] w-24 h-52 opacity-[0.05] text-primary" viewBox="0 0 100 220" fill="none" aria-hidden="true">
      ...
      {/* African-line horizontal accent */}
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-center min-h-screen py-24 lg:py-0">
          ...
          <div className="order-2 lg:order-1 space-y-8 pb-16 lg:pb-32">
            ...
            <h1 className="text-5xl md:text-6xl lg:text-7xl leading-none tracking-wider">
              ...
              <br />
              <span className="text-gradient-gold">É tempo de renovar com presença e identidade.</span>
            </h1>
            <p className="font-body text-lg md:text-xl text-muted-foreground max-w-lg leading-relaxed">
              Um novo jeito de fazer política, com compromisso real com as pessoas, com o território e com o futuro do Paraná.
            </p>
          ...
          <div className="order-1 lg:order-2 flex justify-center lg:justify-end items-end relative self-end">
            ...
            {/* Geometric diamonds behind candidate */}
            <svg className="absolute -right-2 top-[5%] w-16 h-[85%] opacity-40 z-0" viewBox="0 0 50 500" fill="none" aria-hidden="true">
              {Array.from({ length: 12 }).map((_, i) => (
                <g key={i} transform={`translate(0, ${i * 42})`}>
                  <rect x="8" y="0" width="18" height="18" transform="rotate(45 17 9)" fill="hsl(var(--primary))" opacity="0.5" />
                  <rect x="25" y="12" width="12" height="12" transform="rotate(45 31 18)" fill="hsl(var(--accent))" opacity="0.6" />
                  <rect x="3" y="22" width="10" height="10" transform="rotate(45 8 27)" fill="hsl(var(--orange))" opacity="0.4" />
                </g>
              ))}
            </svg>
            ...
            <img
              ...
              className="relative z-10 w-[280px] md:w-[380px] lg:w-[460px] xl:w-[520px] max-h-[85vh] object-contain object-bottom"
              style={{
                filter: "drop-shadow(0 8px 40px rgba(242, 201, 76, 0.25))",
              }}
            />

            {/* Bottom diamond row under candidate */}
            <svg className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-72 h-10 opacity-50 z-20" viewBox="0 0 280 40" fill="none" aria-hidden="true">
              {Array.from({ length: 11 }).map((_, i) => (
                <rect
                  key={i}
                  x={2 + i * 25} y="8" width="16" height="16"
